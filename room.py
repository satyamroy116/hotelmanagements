...
#   variables
        ...
        combo_search = ttk.Combobox(
            Table_Frame, textvariable=self.search_var, font=("arial", 12, "bold"), width=16)
        ...
        txtsearch = ttk.Entry(Table_Frame, textvariable=self.txt_search, font=(
            "arial", 13, "bold"), width=16)
        txtsearch.grid(row=0, column=2)

        btnsearch = Button(Table_Frame, text="search",command=self.txt_search, font=(
            "arial", 11, "bold"), bg="black", fg="gold", width=10)
...
# kam ni kr rha h
    def search(self):
        conn = mysql.connector.connect(host="localhost", username="root", password="1234", database="World")
        my_cursor = conn.cursor()
        my_cursor.execute("select * FROM world.room where"+str(self.search_var.get())+ "LIKE '%" +str(self.txt_search.get())+"%'")
        rows=my_cursor.fetchall()
        if len(rows)!=0:
         self.room_table.delete(*self.room_table.get_children())
         for i in rows:
